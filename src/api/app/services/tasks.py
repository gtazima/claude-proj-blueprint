...
Concentra a lógica de negócio das tarefas (criação, edição, conclusão,
adiamento com justificativa, soft delete e restauração). A camada HTTP
...
class TaskService:
    ...
    def restore(self, task_id: UUID, *, device_id: str | None = None) -> Task:
        """Restaura tarefa soft-deleted."""
        task = self.session.get(Task, task_id)
        if task is None:
            raise TaskNotFoundError(f"Task {task_id} not found")
    ...
    def undo_completion(self, task_id: UUID, *, device_id: str | None = None) -> Task:
        """Reabre tarefa concluída — sem restrição de janela de tempo."""
        task = self.get(task_id)
